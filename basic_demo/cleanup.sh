...
if [ "${1:-}" = "--reset" ]; then
    ...
    sed -i '' 's|^BUCKET=.*|BUCKET="rajatarya/nvidia-demo-basics"|' demo_02_bucket_basics.sh
    sed -i '' 's|^BUCKET=.*|BUCKET="rajatarya/nvidia-demo-mount"|' demo_03_hf_mount.sh
    sed -i '' 's|^BUCKET=.*|BUCKET="rajatarya/nvidia-demo-dataset"|' demo_04_dataset_sync.sh
    sed -i '' 's|^BUCKET = .*|BUCKET = "rajatarya/nvidia-demo-dedup"|' demo_01_xet_dedup.py
    sed -i '' "s|[a-zA-Z0-9_-]*rajatarya/nvidia-demo-|rajatarya/nvidia-demo-|g" cleanup.sh
